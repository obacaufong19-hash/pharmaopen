...
const CountdownStatus = ({ closingTime, isOpen, freeMedicine }: { closingTime: string, isOpen: boolean, freeMedicine: boolean }) => {
...
function PharmacyAppContent() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // If we scroll down more than 80px, hide. If up, show.
  ...
  const handleGetDirections = (address: string) => {
    // Standard Google Maps directions URL from current location to destination
  ...
  return (
    <div className={`${isDarkMode ? 'dark bg-[#0b0b0d] text-white' : 'bg-[#f4f4f9] text-black'} min-h-screen transition-colors duration-500 pb-32 font-sans`}>
      ...
      <div className="sticky top-0 z-50">
        <div className={`${isDarkMode ? 'bg-[#0b0b0d]/80 border-zinc-800' : 'bg-[#f4f4f9]/80 border-white'} backdrop-blur-3xl p-6 border-b shadow-sm`}>
          ...
          <div className="flex gap-3 items-center">
            ...
            />
            <button 
              onClick={() => setOnlyFreeMeds(!onlyFreeMeds)}
              className={`px-4 h-[54px] rounded-[22px] border flex items-center gap-2 transition-all active:scale-95 ${onlyFreeMeds ? 'bg-blue-600 border-blue-600 text-white' : 'bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 text-zinc-500'}`}
            >
        ...
        {activeTab === 'pharmacy' ? (
          <div className="grid gap-5">
            {isLoading ? (
              ...
              <div className="text-center py-20 opacity-40 text-xs font-bold italic tracking-tight">No pharmacies found in this area.</div>
            ) : filteredPharmacies.map((p: any) => (
              <div key={p.id} className={`p-7 rounded-[40px] border transition-all active:scale-[0.98] ${isDarkMode ? 'bg-zinc-900/40 border-zinc-800/50 shadow-2xl' : 'bg-white border-zinc-100 shadow-md'}`}>
                <div className="flex justify-between items-start mb-8">
                  <div>
                    <CountdownStatus 
                      closingTime={p.closing_time || "17:00"} 
                      isOpen={p.is_open} 
                      freeMedicine={p.free_medicine}
                    />
                ...
                <div className="flex gap-3">
                  {/* WHATSAPP LINK */}
                  <a href={`https://wa.me/${p.phone_number?.replace(/\s/g, '')}`} className={`flex-1 py-4 rounded-[20px] border flex justify-center items-center active:scale-95 transition-all ${isDarkMode ? 'bg-zinc-800/40 border-zinc-700' : 'bg-green-50 border-green-100'}`}>
                  ...
                  <button 
                    ...
                    className={`flex-[2.5] py-4 rounded-[20px] border flex justify-center items-center gap-3 active:scale-95 transition-all ${isDarkMode ? 'bg-zinc-800 text-zinc-300 border-zinc-700' : 'bg-zinc-100 border-zinc-200 text-zinc-600 shadow-sm'}`}
                  >
          ...
          <div className="py-20 text-center opacity-30 font-black uppercase tracking-[0.5em] text-[10px]">Interface Locked</div>
        )}
      </main>
      ...
      <nav 
        className={`fixed bottom-10 left-1/2 -translate-x-1/2 w-[92%] max-w-lg z-[100] transition-all duration-500 ease-in-out ${
          showNav ? 'translate-y-0 opacity-100' : 'translate-y-28 opacity-0 pointer-events-none'
        }`}
      >
        <div className={`${isDarkMode ? 'bg-zinc-900/90 border-zinc-800' : 'bg-white/95 border-white'} backdrop-blur-3xl rounded-[44px] border p-2.5 flex items-center justify-around shadow-[0_20px_50px_rgba(0,0,0,0.15)]`}>
