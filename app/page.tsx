...
function PharmacyAppContent() {
  ...
  return (
    <div className={`${isDarkMode ? 'dark bg-[#0b0b0d] text-white' : 'bg-[#f4f4f9] text-black'} min-h-screen transition-colors duration-500 pb-32 font-sans overflow-x-hidden`}>
      ...
      <div className="sticky top-0 z-50">
        <motion.div 
          ...
          className={`${isDarkMode ? 'bg-[#0b0b0d]/80 border-zinc-800' : 'bg-[#f4f4f9]/80 border-white'} backdrop-blur-3xl p-6 border-b shadow-sm`}
        >
          <header className="flex justify-between items-center mb-6">
            ...
            <div className="flex gap-2">
              <motion.button 
                whileTap={{ scale: 0.9 }}
                onClick={() => { triggerHaptic(5); refetch(); }} 
                className={`w-10 h-10 rounded-2xl flex items-center justify-center border transition-all ${isDarkMode ? 'bg-zinc-900 border-zinc-800 text-zinc-400' : 'bg-white border-zinc-200 text-zinc-500 shadow-sm'}`}
              >
              ...
              <motion.button 
                whileTap={{ scale: 0.9 }}
                onClick={() => { triggerHaptic(10); setIsDarkMode(!isDarkMode); }} 
                className={`w-10 h-10 rounded-2xl flex items-center justify-center border transition-all ${isDarkMode ? 'bg-zinc-900 border-zinc-800 text-orange-400' : 'bg-white border-zinc-200 text-blue-600 shadow-sm'}`}
              >
                {isDarkMode ? '🌙' : '☀️'}
              </motion.button>
            </div>
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar mb-4">
            {['All', 'NAVUA', 'NASINU', 'NAUSORI', 'SUVA'].map((loc) => (
              <motion.button 
                key={loc} 
                whileTap={{ scale: 0.95 }}
                onClick={() => { triggerHaptic(5); setSelectedLocation(loc); }}
          ...
          <div className="flex gap-3 items-center">
            <input 
              type="text" 
              placeholder="Search..." 
              className={`flex-1 p-4 rounded-[22px] border shadow-inner outline-none text-sm font-bold transition-all ${isDarkMode ? 'bg-zinc-900/50 border-zinc-800 text-white' : 'bg-white border-zinc-200 text-black'}`}
              onChange={(e) => setSearchQuery(e.target.value)}
            />
            <motion.button 
              whileTap={{ scale: 0.95 }}
              onClick={() => { triggerHaptic(15); setOnlyFreeMeds(!onlyFreeMeds); }}
              className={`px-4 h-[54px] rounded-[22px] border flex items-center gap-2 transition-all ${onlyFreeMeds ? 'bg-blue-600 border-blue-600 text-white shadow-md' : 'bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 text-zinc-500'}`}
            >
              <span className="text-[10px] font-black uppercase tracking-tighter whitespace-nowrap">Free Meds</span>
              <div className={`w-2 h-2 rounded-full ${onlyFreeMeds ? 'bg-white animate-pulse' : 'bg-zinc-300'}`} />
            </motion.button>
          </div>
        </motion.div>
      </div>
          ...
          <AnimatePresence mode="popLayout">
            {filteredPharmacies.map((p: any) => (
              <motion.div 
                ...
                key={p.id} 
                className={`p-7 rounded-[40px] border transition-all ${isDarkMode ? 'bg-zinc-900/40 border-zinc-800/50 shadow-2xl' : 'bg-white border-zinc-100 shadow-md'}`}
              >
                <div className="flex justify-between items-start mb-8">
                  <div>
                    <div className="flex flex-wrap items-center gap-3 mb-2">
                      ...
                      {p.free_medicine && (
                        <span className="px-2 py-0.5 rounded-md bg-blue-500/10 text-blue-500 text-[8px] font-black uppercase tracking-widest border border-blue-500/20">
                  ...
                  <motion.a 
                    ...
                    className="w-14 h-14 rounded-full bg-blue-500/10 text-blue-500 border border-blue-500/20 flex items-center justify-center shadow-inner"
                  >
                ...
                <div className="flex gap-3">
                  <motion.a 
                    whileTap={{ scale: 0.95 }}
                    href={`https://wa.me/${p.phone_number?.replace(/\s/g, '')}`} 
                    className={`flex-1 py-4 rounded-[20px] border flex justify-center items-center ${isDarkMode ? 'bg-zinc-800/40 border-zinc-700' : 'bg-green-50 border-green-100'}`}
                  >
                  ...
                  <motion.button 
                    ...
                    className={`flex-[2.5] py-4 rounded-[20px] border flex justify-center items-center gap-3 ${isDarkMode ? 'bg-zinc-800 text-zinc-300 border-zinc-700' : 'bg-zinc-100 border-zinc-200 text-zinc-600 shadow-sm'}`}
                  >
      ...
      <AnimatePresence>
        {showNav && (
          ...
          >
            <div className={`${isDarkMode ? 'bg-zinc-900/90 border-zinc-800' : 'bg-white/95 border-white'} backdrop-blur-3xl rounded-[44px] border p-2.5 flex items-center justify-around shadow-[0_20px_50px_rgba(0,0,0,0.2)]`}>
              {['pharmacy', 'emergency', 'pro'].map((tab) => (
                <motion.button 
                  key={tab} 
                  whileTap={{ scale: 0.9 }}
                  onClick={() => { triggerHaptic(5); setActiveTab(tab); }} 
                  className={`px-7 py-3 rounded-[32px] transition-all duration-300 font-black text-[9px] uppercase tracking-widest ${activeTab === tab ? 'text-blue-500 bg-blue-500/10 shadow-inner' : 'text-zinc-500'}`}
                >
                  {tab}
                </motion.button>
              ))}
