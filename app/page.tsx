    ...
    <div className={`${isDarkMode ? 'dark bg-[#09090b] text-white' : 'bg-[#F2F2F7] text-black'} min-h-screen transition-colors duration-500 pb-40`}>
        ...
        <div className={`${isDarkMode ? 'bg-[#09090b]/90 border-zinc-800/50' : 'bg-[#F2F2F7]/90 border-zinc-200'} backdrop-blur-2xl p-6 pb-4 border-b`}>
          <header className="mb-6 flex justify-between items-center">
            ...
            <div className="flex gap-2">
              <button onClick={() => refetch()} className={`w-11 h-11 rounded-2xl flex items-center justify-center border transition-all ${isFetching ? 'animate-spin' : 'active:scale-90'} ${isDarkMode ? 'bg-zinc-900 border-zinc-800 text-zinc-400' : 'bg-white border-zinc-200 text-zinc-600'}`}>
            ...
            <>
              <div className="flex gap-2 overflow-x-auto no-scrollbar mb-4">
                {['All', 'NAVUA', 'NASINU', 'NAUSORI'].map((loc) => (
                  <button key={loc} onClick={() => setFilter(loc)} className={`px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest transition-all shrink-0 ${filter === loc ? 'bg-blue-600 text-white shadow-[0_0_15px_rgba(37,99,235,0.4)]' : (isDarkMode ? 'bg-zinc-900 text-zinc-500' : 'bg-white text-zinc-400')}`}>{loc}</button>
                ))}
              </div>
              <input type="text" placeholder="Search directory..." className={`w-full p-4 rounded-3xl border-none shadow-sm outline-none text-sm font-bold transition-all ${isDarkMode ? 'bg-zinc-900/60 text-white placeholder:text-zinc-700' : 'bg-white text-black'}`} onChange={(e) => setSearch(e.target.value)} />
            </>
          )}
        </div>
      </div>
      ...
      <main className="max-w-xl mx-auto p-6">
        {activeTab === 'pharmacy' ? (
              ...
              filteredItems.map((p: any) => (
                <div key={p.id} className={`p-6 rounded-[40px] border transition-all duration-700 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'} ${isDarkMode ? 'bg-zinc-900/40 border-zinc-800/50' : 'bg-white border-transparent shadow-sm'}`}>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <a href={`tel:${p.phone_number}`} className="w-12 h-12 rounded-full flex items-center justify-center bg-blue-500/10 text-blue-400 border border-blue-500/20 shadow-inner">
                  ...
                  <div className="flex gap-2">
                    <button className="flex-1 py-4 rounded-2xl flex items-center justify-center gap-3 text-[10px] font-black bg-green-500/5 text-green-500 border border-green-500/10">WA</button>
                    <button className="flex-1 py-4 rounded-2xl flex items-center justify-center gap-3 text-[10px] font-black bg-purple-500/5 text-purple-400 border border-purple-500/10">VIBER</button>
                    <button className="flex-1 py-4 rounded-2xl flex items-center justify-center gap-3 text-[10px] font-black bg-zinc-800 text-zinc-400">MAP</button>
                  </div>
                </div>
              ))
            )}
          ...
          <div className={`w-full h-[70vh] rounded-[40px] overflow-hidden border shadow-2xl relative ${isDarkMode ? 'border-zinc-800' : 'border-zinc-200 bg-white'}`}>
             <iframe 
                width="100%" 
                height="100%" 
                frameBorder="0" 
                style={{ border: 0, filter: isDarkMode ? 'invert(90%) hue-rotate(180deg) brightness(1.2)' : 'none' }}
                src="https://www.openstreetmap.org/export/embed.html?bbox=178.14, -18.25, 178.50, -18.00&layer=mapnik&marker=-18.226,178.165"
                allowFullScreen
              />
              <div className="absolute top-6 left-6 right-6 p-4 bg-blue-600 rounded-3xl shadow-xl flex items-center gap-3 animate-bounce">
                <div className="w-2 h-2 bg-white rounded-full animate-ping" />
                <p className="text-[10px] font-black text-white uppercase tracking-widest">Live: Navua Community Map</p>
              </div>
          </div>
        )}
      </main>
