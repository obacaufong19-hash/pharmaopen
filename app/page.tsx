...
// Premium Branded Splash Screen with Progress Bar
              ...
              {pharmacies.filter((p: any) => p.name.toLowerCase().includes(search.toLowerCase())).map((p: any) => (
                <motion.div key={p.id} variants={cardVariants} initial="hidden" animate="show" className={`p-7 rounded-[40px] border ${isDarkMode ? 'bg-zinc-900/40 border-zinc-800' : 'bg-white border-zinc-100 shadow-lg'}`}>
                  ...
                  <div className="grid grid-cols-4 gap-3">
                    <a href={`tel:${p.phone_number}`} className="h-14 flex items-center justify-center bg-zinc-800/50 rounded-2xl hover:bg-zinc-700">📞</a>
                    <a href={`https://wa.me/${p.phone_number}`} className="h-14 flex items-center justify-center bg-green-500/10 text-green-500 rounded-2xl hover:bg-green-500/20">💬</a>
                    <a href={`viber://add?number=${p.phone_number}`} className="h-14 flex items-center justify-center bg-purple-500/10 text-purple-500 rounded-2xl hover:bg-purple-500/20">V</a>
                    <button onClick={() => window.open(`https://maps.google.com/?q=${encodeURIComponent(p.address)}`, '_blank')} className="h-14 flex items-center justify-center bg-blue-600 text-white rounded-2xl hover:bg-blue-700">📍</button>
                  </div>
                </motion.div>
              ))}
            </div>
