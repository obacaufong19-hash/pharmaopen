                ...
                return (
                  <div key={p.id} style={{ transitionDelay: `${index * 50}ms` }} className={`p-5 rounded-[32px] flex flex-col gap-4 border transition-all duration-700 transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'} ${isDarkMode ? 'bg-zinc-900/40 border-zinc-800/50' : 'bg-white border-transparent shadow-sm'} hover:scale-[1.005] active:scale-[0.99]`}>
                    ...
                    <div className="flex gap-2 border-t pt-4 dark:border-zinc-800/50">
                      <a href={`https://wa.me/${cleanPhone}`} target="_blank" className={`flex-1 py-3 rounded-2xl flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-wider transition-all ${isDarkMode ? 'bg-green-500/10 text-green-400' : 'bg-green-50 text-green-700'}`}>WA</a>
                      <a href={`viber://contact?number=${cleanPhone}`} className={`flex-1 py-3 rounded-2xl flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-wider transition-all ${isDarkMode ? 'bg-purple-500/10 text-purple-400' : 'bg-purple-50 text-purple-700'}`}>Viber</a>
                      <a href={`https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(p.name + ' ' + p.address)}`} target="_blank" className={`flex-1 py-3 rounded-2xl flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-wider transition-all ${isDarkMode ? 'bg-zinc-800 text-zinc-300' : 'bg-zinc-100 text-zinc-600'}`}>Map</a>
                    </div>
                  </div>
                );
              })
