...
function VitiPulseContent() {
  ...
  const { data: facilities = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Using 'directory' as the table name for the expanded version
          ...
          {activeTab === 'directory' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <div className="flex gap-2 overflow-x-auto no-scrollbar mb-4">
                {['All', 'NAVUA', 'NASINU', 'NAUSORI', 'SUVA', 'NADI', 'LAUTOKA'].map((loc) => (
              ...
              <input type="text" placeholder={`Search ${activeCategory}...`} className={`w-full p-4 rounded-[22px] border outline-none text-sm font-bold ${isDarkMode ? 'border-zinc-800 bg-zinc-900/50 text-white' : 'border-zinc-100 bg-white shadow-inner text-black'}`} onChange={(e) => setSearchQuery(e.target.value)} />
            </motion.div>
          )}
        </div>
      </div>
              ...
              {filteredFacilities.length > 0 ? filteredFacilities.map((f: any) => (
                <motion.div layout variants={cardVariants} key={f.id} className={`p-7 rounded-[40px] border ${isDarkMode ? 'border-zinc-800/50 bg-zinc-900/40 shadow-2xl' : 'border-zinc-100/40 bg-white shadow-lg'}`}>
                  <div className="flex justify-between items-start mb-8 gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className={`text-[10px] font-black uppercase tracking-widest ${f.is_open ? 'text-green-500' : 'text-red-500'}`}>{f.is_open ? 'Open Now' : 'Closed'}</span>
                      </div>
                  ...
                  <div className="grid grid-cols-4 gap-3">
                    ...
                    <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(f.name + ' ' + f.address)}`} target="_blank" rel="noreferrer" className={`h-14 rounded-2xl flex items-center justify-center transition-all ${isDarkMode ? 'bg-blue-600/20 text-blue-400' : 'bg-blue-600 text-white shadow-lg'}`}>
                ...
                <div className="text-center py-20 opacity-50">No {activeCategory} found.</div>
              )}
            </AnimatePresence>
          </motion.div>
        ) : (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="py-10 text-center">
            <div className="w-20 h-20 bg-blue-600 rounded-3xl mx-auto mb-6 flex items-center justify-center text-3xl shadow-2xl">🇫🇯</div>
            <h2 className="text-3xl font-black mb-2 tracking-tight uppercase">Viti Pulse</h2>
            <p className="text-zinc-500 text-sm max-w-sm mx-auto leading-relaxed">The ultimate lifestyle directory for the Fiji islands.</p>
            <div className="mt-12 space-y-4 text-left">
              <div className={`p-7 rounded-[35px] border ${isDarkMode ? 'bg-zinc-900/40 border-zinc-800' : 'bg-white border-zinc-100 shadow-sm'}`}>
                <h3 className="font-black text-[10px] uppercase tracking-[0.2em] text-blue-500 mb-3">Community First</h3>
                <p className="text-sm font-medium leading-relaxed opacity-80">Real-time status updates for pharmacies, supermarkets, and local retail shops.</p>
              </div>
            </div>
          </motion.div>
        )}
          ...
          >
            <div className={`${isDarkMode ? 'bg-zinc-900/95 border-zinc-800 shadow-2xl' : 'bg-white/95 border-white shadow-2xl'} backdrop-blur-3xl rounded-[44px] border p-2 flex items-center justify-around`}>
              <button onClick={() => setActiveTab('directory')} className={`flex-1 flex flex-col items-center gap-1.5 py-3 rounded-[32px] transition-all ${activeTab === 'directory' ? 'bg-blue-600 text-white shadow-lg' : 'text-zinc-500'}`}>
                <span className="text-[10px] font-black uppercase tracking-widest">Browse</span>
              </button>
